...
require './services/NotasCreditoService.php';
...
$sql = "SELECT nc.fecha, nc.cliente, nc.status,nc.relacioncfdi,
        ...
        cia.rfc emisor, cli.nombre, nc.uuid, IFNULL( nc.usr, 'Unknown' ) usr
  	FROM nc 
        JOIN cia ON TRUE
        LEFT JOIN cli ON cli.id = nc.cliente 
        LEFT JOIN facturas ON facturas.id_fc_fk = nc.id AND facturas.uuid = nc.uuid 
        WHERE nc.id = " . $busca;
$CpoA = $mysqli->query($sql);
$Cpo = $CpoA->fetch_array();

if ($Cpo["status"] != StatusNotaCredito::ABIERTO) {
...
$Mensaje = "";
$Cancelar = false;
if ($Cpo["status"] == StatusNotaCredito::CANCELADO && (empty($Cpo['uuid']) || $Cpo['uuid'] === NcDAO::SIN_TIMBRAR)) {
...
} else if (!empty($pagos["pagos"]) && $pagos["pagos"] != 0) {
    ...
    $Mensaje = "El SAT indica que la factura no puede ser cancelada ya que tiene folios relacionados que deben ser cancelados previamente ";
    ...
    <head>
        ...
        <script>
            $(document).ready(function () {
                $("#busca").val("<?= $busca ?>");
                if ("<?= $Cpo["relacioncfdi"] ?>" === "0") {
                    $("#TipoCancelacion").val("02");
                } else {
                    $("#TipoCancelacion").val("01");
                }
            });
        </script>
    </head>
        ...
        <table style="width: 100%;" class="texto_tablas" aria-hidden="true">
            <tr>
                <td style="text-align: center;" class="nombre_cliente">
                    <a href="notascre.php"><img src="libnvo/regresa.jpg" alt="Flecha regresar"></a><br/>regresar
                </td>
                <td style="vertical-align: top;">
                    <form name="form1" id="form1" method="post" action="">
                        ...
                        <?php
                        cTable("99%", "0");
                        cInput("Folio :", "Text", "5", "Id", "right", $busca, "40", false, true, "");
                        ...
                        if (!empty($Cpo['uuid']) && $Cpo['uuid'] !== NcDAO::SIN_TIMBRAR) {

                            cInput("Para su verificación fiscal:", "Text", "0", "Mensaje", "right", "<a class='textosCualli' target='_BLANK' href='https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx"
                                    . "?id=" . $Cpo['uuid']
                                    . "&re=" . $Cpo['rfcEmisor']
                                    . "&rr=" . $Cpo['rfcReceptor']
                                    . "&tt=" . $Cpo['cfditotal']
                        ...
                        if ($Cancelar) {
                            if ($statusCFDI['Estado'] !== "No Encontrado") {
                                ...
                                $CancelacionVO = $CancelacionDAO->retrieve($busca, "nc");
